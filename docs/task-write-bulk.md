...
`Task Write Bulk` 是 AIM Coordinator 面向人工阅读与审批的 Task Pool 写入意图列表。它把 Manager Report、最新基线事实、当前未完成 Task Pool 与 rejected Task 反馈，收敛成一组可检查、可排序、可路由的候选写入。
...
`Create` 的判断依据应来自可引用事实：Manager Report 的差距、最新基线的可观测事实、当前 Task Pool 未覆盖的缺口，或 rejected Task 暴露出的可修复前提缺口。
...
`Delete` 只能作用于明确的未完成 Task。已 `succeeded`、`resolved` 或 `done = 1` 的 Task 属于历史结果，不应通过 `Task Write Bulk` 删除来维护 Task Pool 视图。
...
- 禁止把 README、Manager Report 或开放问题不清晰的内容包装成“澄清类 Developer Task”。
...
- id: create-contract-doc
  action: Create
  depends_on: []
  reason: Manager Report 指出 Coordinator 输出缺少可引用的 Task Write Bulk 审批契约。
