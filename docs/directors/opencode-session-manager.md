...
- `project_id`: AIM Project ID。非空外键，关联到 `projects` 表，父表记录删除时直接级联删除。这个字段表示这个 Session 属于哪个 AIM Project。由于 OpenCode 的限制，每个 Session 必须运行在一个特定的目录下，因此我们需要通过这个字段来确定 Session 的运行目录。
- `continue_prompt`: 用于推进 Session 的 prompt。一旦 Session Manager 认为 Session 需要被推进，就会发送这个 prompt。这个 prompt 的信息量必须是足够从任务开始推进到结束的。
...
- `input_tokens`: Session 输入的非缓存的 token 数量。整数。默认为 0。
...
3. 清理空悬 Sessions。这是指 AIM `opencode_sessions` 表中是 pending，但是执行环境不可靠。
...
2.  第二类错误
    ...
    这个代价会因为过度频繁错误推进而放大，甚至在极端情况下可能会导致 Session Compacting 失败，导致整个 Session 崩溃。因此要着力减少这种错误的发生。但没有必要完全消除。在实际工作中，卡在长工具调用的频率并不高，因此这个错误的发生频率也不会太高。
...
**AIM 永远不允许 OpenCode Session Manager 之外的模块来推进 Session**。我曾经考虑过暴露一个方法，让人类允许通过 GUI 主动推进 Session，为了给无法判断 idle 状态的 Session 提供一个干预兜底的手段。但是这个接口后来就被滥用了，导致其他模块可能错误地利用这个接口无限地推进 Session。这可能会导致 Token 预算失控，或者导致过度频繁推进 Session，引发第二类错误的代价被放大。在严格的机制下，Session 的推进完全由 Session Manager 控制，不允许其他模块来推进。
...
空悬 Session 是指，在 `opencode_sessions` 表中是 pending，但是在实际指向的环境已经不可靠了。

1. 通过 OpenCode API (`GET /sessions/<session_id>`) 来检查 Session 是否存在。如果返回 404 就说明 Session 已经被删除了，这是空悬 Session 的一种情况。
2. 如果 Session 的 `directory` 字段不等于 AIM Project 计算出来的 Session 运行目录 (`resolveProjectWorkspacePath`)，AIM 也会认为它是一个空悬 Session。因为继续推进它所需的环境已经不存在了。
...
外部模块（例如 AIM Developer）需要创建 Session 时，必须调用 OpenCode Session Manager 暴露的 `createSession` 方法来创建 Session。这个方法会在 `opencode_sessions` 表中创建一条记录，并且调用 OpenCode API 来创建 Session。

参数：

- `title`: Session 的标题，必填。这个字段会被同步存储到 OpenCode 中。这个字段方便我们在 GUI 中展示 Session 的标题。
- `project_id`: AIM Project ID，必填。这个字段表示这个 Session 属于哪个 AIM Project。由于 OpenCode 的限制，每个 Session 必须运行在一个特定的目录下，因此我们需要通过这个字段来确定 Session 的运行目录。
- `prompt`: 用于推进 Session 的 prompt，必填。这个 prompt 的信息量必须是足够从任务开始推进到结束的。

不允许添加其他任何字段，包括但不限于下列字段：

- `directory`: Session 的运行目录，通过 `project_id` 计算出来的 AIM Project 工作空间路径。可以通过 `resolveProjectWorkspacePath(project_id)` 来计算得到。这个字段会被传给 OpenCode API 来创建 Session。
- `model`: Session 使用的模型。它可以通过 `projects` 表中的 `resolveProjectModel(project_id)` 来计算得到（背后通过查询 `projects` 表的 `global_provider_id` 和 `global_model_id`），无须外部传入。

返回值仅限于:

- `session_id`: OpenCode Session ID。

不允许返回其他任何字段和方法（包括 Disposable / AsyncDisposable 等语义）。
