...
AIM 当前通过 Prompt 直接注入通过 curl 访问的 AIM API (`/opencode/sessions/<session_id>/resolve` 和 `/opencode/sessions/<session_id>/reject`) 来中止 Session 并给出结果或者原因。这个路径不依赖任何 OpenCode 插件，部署和调试都只需要 AIM Server 与 OpenCode API。
