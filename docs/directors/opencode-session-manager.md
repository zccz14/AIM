...
- 不依赖 OpenCode Plugin 提供的 `session.idle` 信号来判断 Session 是否需要被推进。（但它可以轻微提升实时性，因为巡检的循环时间并不长，这个提升不大，可以不做）
- 不会因为立即的错误（API Quota 不足 / 网络不足）等原因频繁重试。

**为什么是 5 分钟**？它不至于太短导致正常的网络延迟和输出延迟导致第二类误判。它也不至于太长导致效率过低。这个时间是基于经验的一个折中值，提升它可以减少第二类错误发生，降低它可以降低第一类错误的代价。

## 何时中止 Session

OpenCode Session Manager 要追加一份 prompt 来引导 Agent 中止 Session。通过注入一个特殊的提示词来告诉 Agent 何时、如何中止 Session。

有两种方式：

1. 通过 OpenCode Plugin 注入的 tools (`aim_session_resolve` 和 `aim_session_reject`) 来中止 Session 并给出结果或者原因。
2. 通过 Prompt 直接注入通过 curl 访问的 AIM API (`/opencode/sessions/<session_id>/resolve` 和 `/opencode/sessions/<session_id>/reject`) 来中止 Session 并给出结果或者原因。

前一种方式的提示词更简单，但是会依赖 OpenCode Plugin，对用户部署不特别友好；后一种方式更通用，不依赖 OpenCode Plugin，但是提示词更长一些，看着不是特别美观。

未来可以中和一下。目前我认为，后一种方式更好，因为 OpenCode Plugin 的开发和部署体验目前还是太差，没有热重载，每次都需要彻底重启 OpenCode 来加载。如果可以，我希望彻底不依赖 OpenCode Plugin，能大幅简化开发和部署流程。
