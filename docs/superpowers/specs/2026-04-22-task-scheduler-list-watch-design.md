...
5. 保留 polling overlap protection，确保上一次扫描未完成时不会并发开始下一次扫描。
...
1. `start({ intervalMs })` 负责启动定时轮询，并在启动时立即触发一次扫描。
2. `scanOnce()` 负责执行单次扫描；若当前已有扫描在进行中，则直接返回当前 in-flight Promise，不再启动新的扫描，避免重叠执行。
3. `stop()` 只负责停止后续定时触发，不打断已经开始的那次扫描。
...
虽然每次扫描内部改为串行，但外部定时轮询仍必须保留 overlap protection。实现上继续保留一个扫描中的 Promise 或等价标记：

1. 若定时器触发时上一轮 `scanOnce()` 尚未完成，则不得再次开启新扫描。
2. `start()` 启动后的首次立即扫描与后续 interval 触发都共用同一保护逻辑。
3. 扫描结束后才允许下一次触发真正开始执行。

这保证 polling 是周期触发的，但扫描执行本身始终不重叠。
...
3. duplicate `session_id` 的正确性测试应迁移到 repository / schema 测试，验证数据库约束对非 `NULL` `session_id` 生效，同时允许多个 `NULL`。

这样可以让 scheduler 测试只验证 scheduler 责任，避免用调度器单测代替数据库一致性测试。

## 迁移 / 兼容性说明

1. 这是一次内部重构，外部保留 `start()` / `stop()` 能力，但单次执行接口应改名为 `scanOnce()` 或同义命名，不再暴露 `runRound()` / `beginRound()` 语义。
2. 行为上，重复 `session_id` 不再由 scheduler 跳过并记录告警；若数据库中仍可能出现该类脏数据，必须先补齐 schema / repository 约束后再删除对应 scheduler 逻辑。
3. 由于扫描改为串行，同一次扫描处理大量任务时总耗时可能增长；本次接受该取舍，以换取更简单的控制流与更稳定的边界。若后续确有吞吐瓶颈，应作为独立设计讨论，而不是在本次重构中预埋复杂并发结构。
