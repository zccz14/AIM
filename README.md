...
1. 人类充当 Director ，编写和维护 README，评审目标评估维度标准。
...
AIM 包含了多项目管理能力。它可以同时维护多个 Git 项目优化。每个项目拥有独立的目标维度、任务池。
...
AIM 应当具有一个可观测系统 (UI)，能够清晰地展示项目、各个维度的评估标准和迭代历史。AIM 的调度器应当能够根据这些指标动态调整任务优先级和资源分配，以最大化推进效率。
...
AIM 优化器可以按 Project 启用和停用。Project CRUD API 会读写 `optimizer_enabled`，服务启动时直接读取该持久化字段来决定是否启动优化器。当优化器正在运行时，三条 lane 会分别维护评估、维护 Task Pool、跟进未完成 Task；任一 lane 失败只影响自身状态，不阻塞其他 lane。Project 还可以配置 Director 授权的累计硬 Token 预算 `token_budget_limit`：当已归集的 Project Token 用量达到或超过该上限时，优化器不会为该 Project 启动新的工作；Director 只能通过显式提高这个有限上限来继续授权，AIM 不会按时间窗口自动刷新或循环获取预算。
...
AIM 会与 OpenCode 深度集成，但方式仅限于 OpenCode API。

使用 `npx skills` 是目前最方便、最通用的安装/分发 SKILL 的方式。

AIM 通过 OpenCode Session API 来管理 OpenCode Session 的生命周期。

见 [OpenCode Session Manager](docs/directors/opencode-session-manager.md) 设计文档，了解 AIM 如何与 OpenCode 集成，以及 AIM 如何管理它创建的 OpenCode Sessions。
...
[AIM Developer](docs/directors/developer.md) 基础设计，定义 AIM Developer 的职责边界。
...
- AIM Skills 入口：[`.agents/skills`](.agents/skills)，通过 `npx skills` 从仓库直接安装。

每个包要考虑到引用边界，通过安装依赖来引用其他包的产物，不得直接跨包引用。
AIM Skills 是仓库级静态分发内容，不是 workspace package，也不是 OpenCode runtime plugin。

## 安装、构建与测试

安装: `pnpm install`

安装 AIM Skills: `npx skills add zccz14/AIM --skill '*' --agent opencode -y` (安装所有 AIM 的 OpenCode Agent Skills 到当前项目 Scope 中，后续升级也可以通过 `npx skills update` 来完成)
...
Director 澄清与调整请求也通过 CLI 记录，保持在 Director 输入、GUI 可观测、Manager 消化评估信号的边界内，不直接替代 Manager / Coordinator / Developer 的执行职责：

```bash
aim director clarifications list --base-url http://localhost:8192 --project-id <project-id>
...
本地观测已落地的 AIM 只读状态时，可以用 CLI 查询当前维度、评估历史、Coordinator dry-run proposal 和项目优化器状态。项目级 Token 用量查询入口是 `project:token-usage`，用于查看 project totals、硬预算状态、task/session attribution 和 partial failure diagnostics：
...
以下文档同样属于 AIM Director 视角的内容，具有同等重要性，建议 AIM Manager & AIM Coordinator 阅读：
