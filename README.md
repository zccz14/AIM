...
**AIM 是面向 README 的软件开发范式**。AIM 认为软件开发的本质是从基线现状向目标状态的逼近与收敛过程。AIM 的范畴是，建立坐标系、定义度量、拆分增量、持续推进、闭环验证，直到目标状态被充分逼近。AIM 的方法论核心是，**把软件开发当作一个连续的、可度量的、可调度的基线迭代过程来管理**。AIM 的目标是，从已有的任何基线触发，持续向目标状态推进。
...
AIM 包含了 Manager, Planner 和 Developer 的职责。
