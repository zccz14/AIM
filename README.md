...
AIM 优化器可以按 Project 启用和停用。Project CRUD API 会读写 `optimizer_enabled`，服务启动时直接读取该持久化字段来决定是否启动优化器。当优化器正在运行时，三条 lane 会分别维护评估、维护 Task Pool、跟进未完成 Task；任一 lane 失败只影响自身状态，不阻塞其他 lane。可以通过 GET `/optimizer/status` 的 API 查询优化器和各 lane 的当前状态。
