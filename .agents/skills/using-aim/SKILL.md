...
- `aim-coordinator-guide`: direct entry when the user wants AIM Coordinator guidance to maintain the Task Pool by producing approvable `POST /tasks/batch` operations from Manager output, latest baseline facts, current Tasks, and rejected Task feedback.
