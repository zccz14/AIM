...
5. 向 AIM 持续上报生命周期事实，并最终通过 AIM Session Settlement Protocol 完成 bound session。
...
- 成功终态通过 AIM Session Settlement Protocol 完成，`value` 必须是非空完成摘要。
- 失败终态通过 AIM Session Settlement Protocol 完成，`reason` 必须是非空失败原因。
...
3. 如果基线验证失败，使用 AIM Session Settlement Protocol 以非空 `reason` 结算为 rejected；不要继续创建 worktree 或进入实现。
...
10. 只有在以上步骤全部完成后，才能使用 AIM Session Settlement Protocol 以非空 `value` 结算为 resolved，并将 session 视为真正完成。
...
- Spec 验证失败：表示 Task Spec 与最新基线不再匹配、关键前提已失效、或继续执行会偏离任务目标。这是任务本身失败，应使用 AIM Session Settlement Protocol 以非空 `reason` 结算为 rejected，并在 `reason` 里写清基线失配原因。
- Spec API 失败、字段级 PUT 失败、AIM Session Settlement Protocol 失败：这是输入链路或上报链路阻塞，不是任务本身失败。必须显式暴露阻塞，停止声称 AIM 已同步成功，但不要把任务误判为失败。
...
- 只有在 PR 终态成立、worktree 已删除、主工作区基线已刷新后，才能使用 AIM Session Settlement Protocol 以非空 `value` 结算为 resolved，并把 bound session 视为真正完成。

## Session 终态

- 成功终态只通过 AIM Session Settlement Protocol 完成，`value` 必须是非空完成摘要。
- 失败终态只通过 AIM Session Settlement Protocol 完成，`reason` 必须是非空失败原因。
...
4. 最终成功完成时：使用 AIM Session Settlement Protocol 以非空 `value` 结算为 resolved。
5. 任务本身失败时：使用 AIM Session Settlement Protocol 以非空 `reason` 结算为 rejected。
...
- 成功终态只能使用 AIM Session Settlement Protocol。
- 失败终态只能使用 AIM Session Settlement Protocol。
- 终态成功的 `value` 必须是非空字符串。
- 终态失败的 `reason` 必须是非空字符串。
- 除非字段级 PUT 或 AIM Session Settlement Protocol 实际成功，否则不要声称 AIM 已拥有最新事实。
...
- 任务失败：工作本身失败，因此应通过 AIM Session Settlement Protocol 发送带非空 `reason` 的终态失败结算。
- 上报失败：字段级 PUT 请求或 AIM Session Settlement Protocol 因网络、超时、连接、5xx 或意外响应等问题失败。不要把这类情况转换成任务失败。
