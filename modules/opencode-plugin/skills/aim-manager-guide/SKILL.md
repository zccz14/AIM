...
Manager 可通过服务端 API 维护项目维度，并对每个维度追加评估记录。不要直接写 SQLite；所有示例均假设 `SERVER_BASE_URL` 默认为 `http://localhost:8192`，`project_path` 必须与被评估项目一致。
...
```bash
curl "${SERVER_BASE_URL:-http://localhost:8192}/dimensions?project_path=/Users/example/project"
...
评估记录是 append-only。`project_path` 必须与目标 Dimension 的 `project_path` 一致；`score` 是 0-100 整数，语义为 `0-20 缺失`、`21-40 初始`、`41-60 可用`、`61-80 稳定`、`81-95 优秀`、`96-100 近似完成`。
