...
这个 skill 是 AIM Manager 的工作入口，也是当前最小 Manager Report 可观察入口。Manager 负责把 README 目标、最新 `origin/main` baseline、可观测性资料、Issues、现有 Tasks / Task Pool、Rejected Task 反馈等环境信息综合成方向判断，输出给 Coordinator 消费的 `Manager Report`。产品语义与交接边界见 `docs/manager-report.md`。
...
- 当前可观察入口是本 packaged skill：安装 `@aim-ai/opencode-plugin` 后，OpenCode 可通过 `aim-manager-guide` 发现并产出 `Manager Report`。
