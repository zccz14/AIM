...
- 禁止把 RED 写成实现耦合测试。RED 也必须遵守 `aim-writing-tests`：验证行为语义，不验证源码片段、私有协作、内部 import、精确 prompt prose 或过量 mock 调用细节。
- 禁止让 RED 测试暗中执行 build、generate、install、format、lint 或其他重型准备流程。RED 所需前置状态必须由被调用的验证命令显式提供。
...
- 遵守 `aim-writing-tests`，优先验证接口、行为和 contract；只有明确 policy / architecture / generated artifact guard 才能使用源码文本断言。
...
- [ ] 每个新增或修复的行为，先有测试。
- [ ] 写 RED 前已加载并遵守 `aim-writing-tests`。
- [ ] 每个测试都先被实际运行过。
- [ ] 每个测试都先因正确原因失败过。
- [ ] 没有把无关失败误当 RED。
- [ ] RED 没有隐藏 build / generate / install / format / lint 等重型准备流程。
- [ ] 没有在测试之前保留或复用实现。
- [ ] GREEN 阶段只写了最小实现。
- [ ] 当前测试通过。
- [ ] 受影响的相关测试通过。
