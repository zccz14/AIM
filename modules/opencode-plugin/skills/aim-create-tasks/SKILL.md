...
- 明确的 `project_id`。
- 明确的开发模型选择：`developer_provider_id` 与 `developer_model_id`。
...
- `project_id` 是什么。
- 下游 Developer 应使用哪个 `developer_provider_id` 与 `developer_model_id`。
...
- 缺少 `developer_provider_id` 或 `developer_model_id`：停止并补齐，不要猜测。
