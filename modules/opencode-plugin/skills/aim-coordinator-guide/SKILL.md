...
这个 skill 是 AIM Coordinator 维护 Task Pool 时的决策入口。Coordinator 的产物不是泛化分析报告，也不是直接执行开发工作，而是一个等待用户批准的 `Task Write Bulk` list。Manager Report 的产品语义见 `docs/manager-report.md`，Task Write Bulk 的独立输出契约见 `docs/task-write-bulk.md`。
