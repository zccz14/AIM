...
- Uses `aim-ask-strategy` as the broad pre-execution discovery entry when the next action is not yet clear, while the other packaged skills stay as direct workflow guides for Manager Report direction setting, Coordinator Task Pool decisions, task creation, README evaluation, spec verification, test writing, developer execution, and repo setup.
